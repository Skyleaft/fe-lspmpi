<script lang="ts">
	import { Home, User, Bell, MessageSquare } from '@lucide/svelte';
	...
	const pageConfig: Record<string, { title: string; subtitle: string }> = {
		'/dashboard': { title: 'Selamat Datang di Dashboard', subtitle: 'Lembaga Sertifikasi Profesi Manajemen Pendidikan Islam' },
	...
	let pageInfo = $derived(pageConfig[$page.url.pathname] || { title: 'Dashboard', subtitle: 'Lembaga Sertifikasi Profesi Manajemen Pendidikan Islam' });
	...
	function handleProfileImageError(event: Event) {
		(event.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=40&h=40&fit=crop&crop=face';
	}
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center space-x-4">
			...
			<div class="relative">
				...
				{#if showProfileMenu}
					<div class="absolute right-0 mt-2 w-56 rounded-lg border border-gray-200 bg-white shadow-lg z-50">
						<div class="p-3 border-b border-gray-100">
							<p class="text-sm font-medium text-gray-900">{authState.user?.profileName || 'User'}</p>
