<script lang="ts">
	...
	import { findArticleTags, createArticleTag, updateArticleTag } from '$lib/utils/api';
	import type { ArticleTag } from '$lib/types';
	import { fly, fade } from 'svelte/transition';
	import { cubicOut } from 'svelte/easing';

	let { tag = null, isOpen = false, onClose, onSave } = $props();
	let name = $state('');
	...
	async function saveTag() {
		try {
			...
			if (tag) {
				// Update existing tag
				await updateArticleTag(tag.id, tagData);
			} else {
				// Create new tag
				await createArticleTag(tagData);
			}

			onSave();
			onClose();
		} catch (err: any) {
			error = err.message || 'Gagal menyimpan tag';
		...
		>
			...
			>
				...
				<div class="space-y-6 p-6">
					...
					<div class="space-y-4">
						<div>
							<label class="mb-1 block text-sm font-medium text-gray-700">Nama Tag</label>
							<input
								type="text"
								bind:value={name}
				...
				<div class="flex justify-end space-x-3 border-t border-gray-200 bg-gray-50 px-6 py-4">
					...
					>
						{isSaving ? 'Menyimpan...' : tag ? 'Update' : 'Simpan'}
					</button>
				</div>
			</div>
		</div>
